function Add-Path {
    # https://stackoverflow.com/questions/69236623/adding-path-permanently-to-windows-using-powershell-doesnt-appear-to-work
    param(
...
$userpath = $(Join-Path -Path $env:USERPROFILE -ChildPath bin)

if (!$(test-path $userpath)) {
    write-host "creating $userpath as does not exist"
    new-item -ItemType Directory $userpath
}
Add-Path -LiteralPath $userpath -Scope CurrentUser
